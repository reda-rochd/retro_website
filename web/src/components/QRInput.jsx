...
export default function QRScanner({ onScan, placeholder="Scan QR Code", className = "" }) {
	...
	return (
		<div className={`${className}`}>
			...
			<div className="fixed inset-0 bg-black/50 flex items-center justify-center" style={{ display: scanning ? 'flex' : 'none' }}>
				<div className="w-74 h-74 bg-white rounded-xl p-2 flex flex-col items-center justify-center relative">
					<div className="w-70 h-70 rounded-xl overflow-hidden">
					<video
						ref={videoRef}
					...
					<button onClick={cancelScan} className="h-8 w-8 bg-white rounded-full text-red-500 absolute -top-2 -right-2 cursor-pointer">X</button>
				</div>
			</div>
